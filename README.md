...
### 생긴 문제점
코드를 다 적고 실행 했을 때, 구문 오류 : 식별자 가 발생하였습니다.

플레이어 클래스의 `attack(Monster* monster)` 와 몬스터 클래스의 `attack(Player* player)` 이 부분입니다.

강의 자료와 검색을 통해 해결은 하였으나, 무슨 이유로 생긴 오류인지 모르기 때문에 튜터님에게 물어보았습니다.

오류가 발생한 이유는 순환 참조로 `Player.h`에서 `Monster.h`를 포함하고, 동시에 `Monster.h`에서 `Player.h`를 포함하면 컴파일러는 무한 루프에 빠지게 됩니다. 

"너를 알려면 나를 알아야 하고, 나를 알려면 너를 알아야 하는" 모순

해결 방법으로는 전방 선언을 활용하여 순환 참조를 방지하고 `Player.cpp` 에`Monster.h` 를

`Monster.cpp` 에 `Player.h`를 추가 했습니다.
